...
* In addition to MATLAB, the following programs and add-ons:
    ...
    * For MathWorks R2024a tools you must use v3.1.1 or newer of RFSoC Explorer
      (**do not use v3.1.0 or older for MathWorks tools R2024a or newer**)
...
.. warning:: For **Matlab R2024a release**,
  you must install RFSoC Explorer v3.1.1 or newer

.. warning:: For **Matlab R2023 and older releases**.
  For R2023b you may use v3.1.0 or v3.1.1, but for R2023a and older
  you must install older compatible versions. Double check Avnet's
  notes in MathWorks Add-On manager for RFSoC Explorer Matlab
  compatiblity.
...
* `Click to download the Avnet HDL Coder for ZCU208 zip file
  <https://github.com/AvnetDev/hdlcoder-zcu208-zip/releases/download/v1.0.4a/zcu208-hdlcoder.zip>`_
...
.. error::
  Do NOT execute installhdl208.m IF you are using RFSoC Explorer version v3.1.1
  or newer! If you are using Matlab R2024a or newer you MUST use v3.1.1 or newer
  ALSO! Explanation: MathWorks has changed APIs somewhat over the years, in R2024a the
  plugin interface changed for RFSoC HDL Coder.  RFSoC Explorer 3.1.1 suports both
  R2023b and r2024a and now conditionally adds the correct plugin version.  The install
  script would interfere with this.  The installhdl208.m script is left behind for
  users that are using older matlab and RFSoC Explorer versions only.
...
For R2024a and newer Matlab versions only, you will need to manually open and edit the interfaces
file:

.. code-block:: matlab

  edit('./sdcard/interfaces')
